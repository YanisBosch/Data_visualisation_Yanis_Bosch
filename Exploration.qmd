...
```{r}
housing_label <- housing_Connecticut_filtered |> filter(year(date_recorded) == 2009) |> summarize(min = min(sale_amount), max = max(sale_amount), med = median(sale_amount)) |> t() |> as_tibble() |> add_column(Type = c("Min","Max","Med"))

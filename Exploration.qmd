...
housing_Connecticut |> arrange(-sales_ratio)
...
housing_Connecticut <- housing_Connecticut |> mutate(property_type = case_when(
                      ...
                      arrange(-assessed_value)
housing_Connecticut
```


```{r}
housing_Connecticut |> filter(property_type == "Residential" | property_type == "Others") |> group_by(year = year(date_recorded),property_type) |> summarize(median = median(sale_amount)) |> ggplot() +
          geom_line(aes(x = year, y = median)) +
          labs(title = "Median price of housing by year",caption = "Connecticut USA") +
...
housing_Connecticut_fitted |> group_by(year = year(date_recorded)) |> summarize(across(c(sale_amount,assessed_value),mean,.names = "median_{.col}")) |> ggplot() +
          ...
          labs(title = "Perceived vs actual value of housing", caption = "Connecticut USA", y = "Price($)", x = "Year") +
...
housing_Connecticut_fitted |> ggplot(aes(x = sale_amount, y = assessed_value)) +
        stat_binhex() +
        labs(title = "Sale price vs assessed value", x = "Sale price", y = "Assessed value",caption = "Connecticut USA") 
```
...
housing_Connecticut_fitted |> mutate(remove = ifelse(((.fitted)/(assessed_value) > 1/r & (.fitted)/(assessed_value) < r),TRUE,FALSE)) |> ggplot(aes(x = sale_amount, y = assessed_value,colour = remove)) +
        stat_binhex() +
        guides(colour = "none") +
        labs(title = "Sale price vs assessed value", x = "Sale price", y = "Assessed value",subtitle = "Outliers", count = "Density",caption = "Connecticut USA") 
...
housing_Connecticut_filtered |> ggplot(aes(x = sale_amount, y = assessed_value)) +
        ...
        xlim(0,xmax) +
        ylim(0,ymax) +
        labs(title = "Sale price vs assessed value", x = "Sale price", y = "Assessed value",subtitle = "Outliers",caption = "Connecticut USA")
```
...
housing_Connecticut_filtered |> group_by(year = year(date_recorded)) |> summarize(across(c(sale_amount,assessed_value),mean,.names = "median_{.col}")) |> ggplot() +
          geom_line(aes(x = year, y = median_assessed_value)) +
          geom_line(aes(x = year, y = median_sale_amount)) +
          labs(title = "Perceived vs actual value of housing", subtitle = "Connecticut USA", y = "Price($)", x = "Year") +
          geom_ribbon(aes(x = year,ymin=median_assessed_value,ymax=median_sale_amount, fill= "Actual value"), alpha=0.5) +
          geom_ribbon(aes(x = year,ymax=median_assessed_value,ymin=min(median_assessed_value), fill= "Perceived value"), alpha=0.5) +
          geom_vline(xintercept=2007,  linetype='dashed', color='red') +
          guides(fill = "none") +
...
housing_Connecticut_filtered|> filter(year(date_recorded) >= 2006 & year(date_recorded) <= 2009) |> group_by(year(date_recorded)) |> 
      ...
      guides(colour = "none") +
      labs(x = "Year",y = "Sale amount(€)", title = "Distribution of house pricing over the years")
```

```{r}
housing_Connecticut |> ggplot() +
  geom_histogram(aes(x = sales_ratio)) +
  scale_x_continuous(trans = 'log10') +
  scale_y_continuous(trans = 'log10')
```
```{r}
housing_Connecticut_filtered |> ggplot() +
  geom_histogram(aes(x = sales_ratio)) +
  scale_x_continuous(trans = 'log10') +
  scale_y_continuous(trans = 'log10')
```
